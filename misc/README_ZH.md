...
如果菜单、按钮或动态面板仍有漏译，可以在网页上点击鼠标右键，选择 FluentRead 的 **“识别全部节点”**。它可以直接使用，也能在全文翻译后补充识别，并保留已有译文。适用场景包括 [Epoch AI 数据中心页面](https://epoch.ai/data/ai-data-centers)、n8n 节点面板以及展开的导航树；启用后新打开的菜单也会自动翻译，结束时仍用原有恢复原文操作恢复网页。识别范围是已挂载、可见的文本 DOM，Canvas 或图片中的文字请使用图片翻译或圈选翻译，详见[功能介绍](https://fluent.thinkstu.com/guide/features#补充翻译菜单和动态界面)。
