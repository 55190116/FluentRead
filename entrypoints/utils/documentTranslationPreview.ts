...
function pairedUnit(
    ...
    const translatedText = inlineMarkdown(removeMarkdownMarker(translation || source, kind));
...
function renderTextPreview(source: string, translated: string, mode: DocumentPreviewMode): string {
    const sourceBlocks = source.replace(/\r\n?/gu, '\n').split(/\n\s*\n/gu).filter((value) => value.trim());
    const translatedBlocks = translated.replace(/\r\n?/gu, '\n').split(/\n\s*\n/gu).filter((value) => value.trim());
...
export function createDocumentPreviewHtml(
    ...
    if (document.format === 'txt') return renderTextPreview(source, translated, mode);
