...
function getWorker(): Worker {
  ...
  worker.onerror = (event) => {
    terminateWorker(new Error(event.message || '本地视频 AI Worker 已停止'));
  };
...
async function transcribeLocalVideoAudioNow(request: LocalVideoTranscriptionRequest): Promise<LocalVideoTranscriptionResult> {
  ...
  if (!response.success) throw new Error(response.error || '本地视频 AI 字幕失败');
  return {
...
async function prepareLocalVideoTranscriptionModelNow(
  ...
  if (!response.success) throw new Error(response.error || '本地视频 AI 字幕模型初始化失败');
  const result = {
    model,
    backend: response.backend === 'webgpu' || response.backend === 'wasm' ? response.backend : undefined,
    gpuInfo: typeof response.gpuInfo === 'string' ? response.gpuInfo : undefined,
...
function drainQueue(): void {
  ...
  const prepareJob = !transcriptionJob && prepareCandidate
    && (!activeStreamId || (prepareCandidate.streamId && prepareCandidate.streamId === activeStreamId))
    ? pendingPrepare.shift() || null
    : null;
    ...
    try {
      if (prepareJob) {
        if (prepareJob.streamId) {
          if (activeStreamId && activeStreamId !== prepareJob.streamId) {
...
export async function cancelLocalVideoTranscription(streamId: unknown): Promise<void> {
