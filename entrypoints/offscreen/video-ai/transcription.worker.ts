...
async function createWasmTranscriber(modelId: string): Promise<LocalTranscriber> {
...
async function createLocalTranscriber(
  ...
  const transcriber = await createWasmTranscriber(modelId);
  ...
  const segments = Array.isArray(output?.chunks)
    ? output.chunks.map((chunk: any) => {
        ...
        let startMs = Math.min(
          audioDurationMs,
          Math.max(0, normalizeTimestamp(timestamp[0], previousEndMs / 1000) * 1000),
        );
        let endMs = Math.min(
          audioDurationMs,
          Math.max(startMs, normalizeTimestamp(timestamp[1], audioSeconds) * 1000),
        );
        // Streaming mode may return a final sentence without a closing
        // timestamp. Keep that sentence instead of collapsing it to 0 ms.
        if (endMs <= startMs && audioDurationMs > 0) {
          startMs = Math.max(0, Math.min(startMs, audioDurationMs - 400));
          endMs = audioDurationMs;
        }
        previousEndMs = Math.max(previousEndMs, endMs);
        return { startMs, endMs, text: cleanTranscriptText(chunk?.text) };
