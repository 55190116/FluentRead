...
export interface AiSdkTranslationRequest {
...
async function translateSingle(
  ...
  const apiKey = config.token[service]?.trim() || '';
...
export async function translateWithOpenAICompatibleAiSdk(
  ...
  const service = request.serviceOverride || config.service;
  const requestBudget = normalizedTimeout(request.requestTimeoutMs);
  if (!Array.isArray(request.origin)) {
    return translateSingle({...request, requestTimeoutMs: requestBudget}, service, request.origin);
  }
