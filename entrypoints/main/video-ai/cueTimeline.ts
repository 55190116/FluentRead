...
// 推理结果迟到时从“真正可用”的播放器时刻开始保证可读时间，不再把旧句
// 无条件挂四秒。这样既不会一闪而过，也不会让已经说完的字幕长期追在画面后。
export const VIDEO_AI_CUE_LATE_GRACE_MS = 700;
...
export function getVisibleVideoAiCue(
  ...
  for (const cue of cues) {
    ...
    const startMs = Math.max(cue.startMs, availableAtMs) - earlyToleranceMs;
    const spokenEndMs = Math.max(
      getAiCueEndMs(cue) + lateGraceMs,
      getAiCueSpokenEndMs(cue) + lateGraceMs,
      availableAtMs + VIDEO_AI_MIN_READABLE_CUE_MS,
    );
    const translationAvailableAtMs = getAiCueTranslationAvailableAtMs(cue);
    const spokenVisible = currentMs >= startMs && currentMs < spokenEndMs;
    const translatedVisible = translationAvailableAtMs > 0
      && currentMs >= translationAvailableAtMs - earlyToleranceMs
      && currentMs < translationAvailableAtMs + VIDEO_AI_MIN_READABLE_CUE_MS;
    if (!spokenVisible && !translatedVisible) continue;
