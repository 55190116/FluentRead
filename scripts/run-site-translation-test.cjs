...
async function toggleHover(page, target, targetConfig, expectedCount, timeout, settleMs = 250) {
...
async function runHoverCase(page, hoverTargets, requiredSelectors, pageContract, timeout, artifactsDir, hoverSettleMs) {
  const initialUrl = page.url();
  const results = [];
  ...
  for (const [targetNumber, targetConfig] of hoverTargets.entries()) {
    ...
    for (const expected of [1, 0, 1]) {
      await toggleHover(page, target, runtimeTargetConfig, expected, timeout, hoverSettleMs);
