...
async function main() {
  ...
  try {
    ...
    const advanced = options.locator('[data-configuration-group="advanced"]');
    const requestGroup = advanced.locator('[data-configuration-group="requests"]');
    ...
    assert.equal(await requestGroup.locator('.connection-field').filter({hasText: 'API 格式'}).locator('.el-select').count(), 1, 'DeepSeek API 格式必须位于请求设置内');
    ...
    const reopenedAdvanced = options.locator('[data-configuration-group="advanced"]');
    if (await reopenedAdvanced.getAttribute('open') === null) await reopenedAdvanced.locator('summary').click();
    ...
    const narrowAdvanced = options.locator('[data-configuration-group="advanced"]');
    const narrowRequestGroup = narrowAdvanced.locator('[data-configuration-group="requests"]');
    await narrowAdvanced.waitFor({state: 'visible'});
    if (await narrowAdvanced.getAttribute('open') === null) await narrowAdvanced.locator('summary').click();
