...
(async () => {
  ...
  try {
    ...
    const deepseekAdvanced = page.locator('[data-configuration-group="advanced"]');
    if (await deepseekAdvanced.getAttribute('open') === null) await deepseekAdvanced.locator('summary').click();
    const deepseekKeys = deepseekAdvanced.locator('[data-configuration-group="keys"]');
    ...
    await deepseekAdvanced.locator(':scope > summary').click();
    ...
    assert.equal(await page.locator('[data-api-key-index]').count(), 2);
    const reopenedDeepseekAdvanced = page.locator('[data-configuration-group="advanced"]');
    if (await reopenedDeepseekAdvanced.getAttribute('open') === null) await reopenedDeepseekAdvanced.locator('summary').click();
    const reopenedDeepseekKeys = reopenedDeepseekAdvanced.locator('[data-configuration-group="keys"]');
    await page.locator('[data-api-key-rotation-setting] .el-switch').click();
    ...
    await page.locator('[data-api-key-rotation-setting] .el-switch').click();
    await reopenedDeepseekAdvanced.locator(':scope > summary').click();
    ...
    assert.equal(await page.locator('.service-catalog').getAttribute('data-default-service'), initialDefault);
    report.crossPageSync = true; report.cases.push('selection-only-keeps-default-service');
    ...
    const serviceAdvanced = page.locator('[data-configuration-group="advanced"]');
    assert.equal(await serviceAdvanced.getAttribute('open'), null); assert.equal(await page.locator('.no-model-panel').count(), 0); report.cases.push('advanced-collapsed-and-no-empty-model-placeholder');
    ...
    const freeAdvanced = serviceAdvanced.locator('[data-configuration-group="requests"]');
    assert.equal(await serviceAdvanced.getAttribute('open'), null);
    assert(await page.locator('[data-fallback-provider]:visible').count() > 0);
    await shot('service-library-free-configuration');
    const sequentialMode = page.locator('input[type="radio"][value="sequential"]');
    assert.equal(await sequentialMode.count(), 1); await sequentialMode.check();
    await serviceAdvanced.locator(':scope > summary').click();
    ...
    await page.locator('[data-fallback-provider="myMemory"] .provider-settings summary').click();
    ...
    await page.close(); page = await newOptions(); await rail().waitFor({state: 'visible'});
    await selected('freeTranslation').click();
    const reopenedOuterAdvanced = page.locator('[data-configuration-group="advanced"]');
    const reopenedFreeAdvanced = reopenedOuterAdvanced.locator('[data-configuration-group="requests"]');
    assert.equal(await reopenedOuterAdvanced.getAttribute('open'), null);
    ...
    if (await reopenedOuterAdvanced.getAttribute('open') === null) await reopenedOuterAdvanced.locator('summary').click();
    await shot('service-library-free-advanced');
    await selected('claude').click();
    assert.equal(await page.locator('[data-configuration-group="advanced"]').getAttribute('open'), null);
