...
(async () => {
  ...
  try {
    ...
    await patch({uiLanguage: 'zh-CN', uiLanguageSetupCompleted: true, on: true, theme: 'light', interfaceSkin: 'default', service: 'freeTranslation', to: 'zh-CN'});
    ...
    await page.locator('[data-popup-quick-feature="selection"]').click();
    ...
    await shot('popup-multiple-menu'); await close(voices);
