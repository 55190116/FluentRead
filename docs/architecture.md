...
- **展示稳定性**：等价双语重挂按来源、结构和译文工件验证后接管；鼠标高亮仅改变绘制，不改变宿主布局。持续与宿主争抢 DOM 时使用已有修复预算收敛，避免无限恢复和重译。宿主只改写复制进骨架的属性（悬停预览增删链接 `title`、roving tabindex 补 `-1/0`、a11y 脚本补 aria 属性）时，工件按“仅属性漂移”保留且不计熔断，并可按当前 DOM 重建骨架复用已提交译文；只有译文内容或 wrapper 外层属性被改写才走篡改预算。
