...
动态权重只存在于当前运行进程，使用摘要隔离服务、端点、模型与 Key。各 Key 初始权重相同；鉴权、配额和限流使其暂停，网络/服务暂时失败逐步降低权重，成功提高权重。默认恢复窗口为 10 分钟，明确的 Retry-After 至少等待 1 秒；取消、模型及公共配置错误不惩罚 Key。每次请求每个唯一 Key 最多尝试一次，各次尝试都受原调度器与共享 deadline 限制。阅读/写作流仅在建立前失败时切换，开始输出后不重放。
