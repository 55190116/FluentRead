...
服务的完整凭据列表使用 `apiKeys[service]`，旧 `token[service]` 是首个非空值的兼容镜像。配置保存、导入、备份、历史恢复和地址绑定同时处理两者；公开导出和历史不包含 Key 列表。旧单 Key 按原值迁移，不按逗号拆分。

动态权重只存在于当前运行进程，使用摘要隔离服务、端点、模型与 Key。各 Key 初始权重相同；鉴权、配额和限流使其暂停，网络/服务暂时失败逐步降低权重，成功提高权重。默认恢复窗口为 1 分钟，用户可在高级请求限制中调整为 1–60 分钟；明确的 Retry-After 至少等待 1 秒并优先于该默认值。取消、模型及公共配置错误不惩罚 Key。每次请求每个唯一 Key 最多尝试一次，各次尝试都受原调度器与共享 deadline 限制。阅读/写作流仅在建立前失败时切换，开始输出后不重放。

连接检测按原始行索引执行且绕过自动切换；空行与重复值不参与批量检查。界面中的勾选只代表该次检查结果，编辑或服务配置变化使结果失效。
