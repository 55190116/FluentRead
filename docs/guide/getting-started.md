...
默认目标仍为简体中文。导入旧配置时，`zh`、`zh-CN`、`zh-SG` 自动归入 `zh-Hans`，`zh-TW`、`zh-HK`、`zh-MO` 自动归入 `zh-Hant`；显式 `Hans` / `Hant` 优先于地区。简繁译文缓存和术语库分别匹配，旧术语库的 `zh` 也归入简体。只有共享汉字或简繁混排时，系统会继续翻译，不凭不确定的检测结果跳过请求。
