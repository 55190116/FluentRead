...
所有 AI 服务都可以在“高级设置”中编辑 system / user 提示词和自定义请求体。user 提示词编辑器支持点击插入 `{{to}}`（目标语言）与 `{{origin}}`（待翻译原文），保存后会在实际请求中替换；system 提示词用于固定的角色、语气和输出规则。

不同供应商对兼容接口的实现并不完全相同。如果请求失败，先用服务商官方示例验证地址、模型和密钥，再回到 FluentRead 检查配置。

### 自定义 OpenAI 兼容服务

如果服务商提供 OpenAI Chat Completions 兼容接口，可以在“翻译服务”的“我的服务”区域点击“+ 添加”，最多保存 20 个自定义服务。创建时填写名称、接口地址、可选 API Key 和首个模型；每个服务会独立保存 endpoint、凭据、代理、system/user 模板、附加请求体以及最多 50 个模型，不会改变其他服务的配置。

需要新增模型时，打开紧凑的模型选择器并点击“+ 添加模型”，输入模型标识后确认添加；已保存模型以短标签排列，可以直接切换或删除，不会让单个模型铺满整行，也不需要在服务表单下方长期保留一个额外输入框。Grok、OpenAI 等内置 AI 服务同样可以各自保存最多 50 个自定义模型。服务和模型设置保存在本地配置中；API Key 单独加密保存在扩展私有存储，并在浏览器重启后自动恢复。填写完成后使用右上角“检查连接”发送一条最小请求确认配置。

旧版只保存一个“自定义接口”的配置会在首次读取时自动迁移为新的服务和模型列表，原有接口地址、模型、凭据、代理、提示词和请求体无需重新填写。
