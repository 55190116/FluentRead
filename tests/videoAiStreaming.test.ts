...
} from '@/entrypoints/main/video-ai/streamingTranscript';
import {
  VIDEO_AI_MAX_CUE_COUNT,
  getVisibleVideoAiCue,
  mergeVideoAiSubtitleCues,
  upsertVideoAiSubtitleCue,
} from '@/entrypoints/main/video-ai/cueTimeline';
...
describe('本地 AI 字幕时间轴稳定性', () => {
  it('时间轴入口也保证至少 1800ms 的实际可见时长', () => {
    const [cue] = mergeVideoAiSubtitleCues([
      { startMs: 1_000, durationMs: 240, text: 'This is readable now.' },
    ]);
    expect(getVisibleVideoAiCue([cue], 1_000)?.text).toBe(cue.text);
    expect(getVisibleVideoAiCue([cue], 2_799)?.text).toBe(cue.text);
  });

  it('迟到结果不会提前显示，并从真正可用时刻起至少可读 1800ms', () => {
    ...
    expect.soft(getVisibleVideoAiCue([cue], 5_000)?.text).toBe(cue.text);
    expect.soft(getVisibleVideoAiCue([cue], 6_799)?.text).toBe(cue.text);
    expect.soft(getVisibleVideoAiCue([cue], 6_800)).toBeNull();
  });

  it('慢译文返回后独立再保证 1800ms 可读时间，不让原文在等待期常驻', () => {
    const cue = {
      startMs: 0,
      durationMs: 1_800,
    ...
    expect(getVisibleVideoAiCue([cue], 3_799)?.text).toBe(cue.text);
    expect(getVisibleVideoAiCue([cue], 8_800)).toBeNull();
    expect(getVisibleVideoAiCue([cue], 9_000)?.text).toBe(cue.text);
    expect(getVisibleVideoAiCue([cue], 10_799)?.text).toBe(cue.text);
    expect(getVisibleVideoAiCue([cue], 10_800)).toBeNull();
  });
