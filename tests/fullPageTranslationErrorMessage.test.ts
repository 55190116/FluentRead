...
describe('全文翻译错误文案', () => {
  it.each([
    ...
    ['模型输出出现重复或不完整，请尝试混元或缩短文本。', '模型输出出现重复或不完整，请尝试混元或缩短文本。'],
    ['本地模型内存不足', '本地模型内存不足'],
    ['model output token limit reached', 'model output token limit reached'],
    ['model request timeout', '请求超时啦，请稍后再试一次。'],
    ['model_not_found', '模型配置可能有误，请前往设置页面进行检查和调整。'],
