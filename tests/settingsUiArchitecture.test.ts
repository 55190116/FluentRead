...
describe('options UI composition architecture', () => {
  ...
  it('owns settings, feature UI and shared components in their target layers', () => {
    ...
    expect(modelUsageDashboard).toContain("action: 'export'")
    expect(modelUsageDashboard).toContain("action: 'import'")
    ...
    expect(modelUsageDashboard).toContain('averageInputTokensPerReportedRequest')
    expect(modelUsageDashboard).toContain('averageOutputTokensPerReportedRequest')
    ...
    expect(modelUsageDashboard).toContain('selectedTotals.cacheTokenHitRate')
    expect(modelUsageDashboard).toContain('selectedTotals.cacheWriteTokens')
    expect(modelUsageDashboard).toContain('逐请求可观测记录')
    expect(modelUsageDashboard).toContain('不保存原文、译文、提示词、网页地址、API Key、请求体或响应正文')
    expect(modelUsageDashboard).toContain('MODEL_USAGE_TRANSFER_FORMAT')
