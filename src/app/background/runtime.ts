 ...
 * 文件职责：作为扩展后台应用的顶层启动函数，按唯一顺序组装 X 首屏激活、标签页状态、右键菜单、消息监听和维护任务。
 * 主要内容：先对账 X/Grok 动态 content script，再创建共享 TabTranslationStateStore，安装菜单与消息 runtime，最后注册备份及缓存清理。
 * 模块边界：本文件只负责后台生命周期启动，不实现动态注册算法、消息分派、菜单规则或缓存逻辑；各子系统由 feature 与相邻模块实现。
 */
...
import {installXGrokPageBridgeRegistration} from '@/src/features/x-grok-translation/background/public';
