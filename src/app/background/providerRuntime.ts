 ...
 * 主要内容：重导出连接测试错误格式化与 Microsoft 翻译，统一注入共享请求 scheduler、最终模型身份、本地用量代次和响应前有限等待的 IndexedDB 记录。
...
/** 输入框翻译与 broker 共用服务 bucket；lease 覆盖真实 Microsoft transport。 */
export async function translateInputBoxWithLimits(text: string, targetLanguage: string): Promise<string> {
    return translationRequestScheduler.schedule(async (lease) => {
        const operation = translateMicrosoftTexts([text], '', targetLanguage);
        lease.holdUntil(operation);
        const translations = await operation;
        return translations[0] || '';
    }, {
        identity: {service: 'microsoft'},
    });
}
