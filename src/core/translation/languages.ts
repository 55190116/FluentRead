 ...
 * 主要内容：定义 TranslationLanguageOverride、TranslationLanguageDefaults、TranslationLanguages，并由 resolveTranslationLanguages 清洗空值、应用回退、归一中文简繁别名和保留明确的 auto 选择。 可核对的公开符号包括 TranslationLanguageOverride、TranslationLanguageDefaults、TranslationLanguages、resolveTranslationLanguages。
...
function readLanguage(value: unknown, fallback: string): string {
    return normalizeChineseLanguageCode(typeof value === 'string' && value.trim().length > 0 ? value : fallback);
}
