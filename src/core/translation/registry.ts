 ...
 * 主要内容：汇集 B 站、Discord、GitHub、GNU、Hacker News、LearnOpenGL、Reddit、Ubuntu 手册、X/X Chat 与 YouTube 适配器，按 priority 和声明顺序生成 defaultTranslationAdapters。 可核对的公开符号包括 defaultTranslationAdapters。
 * 模块边界：本文件属于可独立测试的 core 候选领域；可以读取传入 DOM 以计算结果，但不访问配置存储、不调用 provider、不注册页面监听器，也不负责译文渲染或 feature 生命周期。
 */

import type {TranslationSiteAdapter} from './types';
import {githubAdapter} from './adapters/github';
import {xAdapter, xChatAdapter} from './adapters/x';
...
import {ubuntuManpageAdapter} from './adapters/ubuntu';
