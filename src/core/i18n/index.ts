...
const legacyPatterns: ReadonlyArray<readonly [RegExp, (match: RegExpExecArray) => string]> = [
    ...
    [/^(.+) 需要 (.+) 和 (.+)，当前尚未完整配置；请先在设置中填写，再开始翻译。$/u, (match) => `${translateLegacyText(match[1], 'en-US')} requires ${translateLegacyText(match[2], 'en-US')} and ${translateLegacyText(match[3], 'en-US')}, which are not fully configured. Add them in settings before translating.`],
    [/^(.+) 需要 (.+)，当前尚未配置；请先在设置中填写，再开始翻译。$/u, (match) => `${translateLegacyText(match[1], 'en-US')} requires ${translateLegacyText(match[2], 'en-US')}, which is not configured. Add it in settings before translating.`],
    [/^输入 (.+)；留空表示尚未配置$/u, (match) => `Enter ${translateLegacyText(match[1], 'en-US')}; leave empty if it is not configured yet`],
...
const esLegacyPatterns: ReadonlyArray<readonly [RegExp, (match: RegExpExecArray) => string]> = [
    ...
    // 云服务厂商的凭据名称由目录提供，用通用模板兜底；具体模板在前面先匹配。
    [/^(.+) 需要 (.+) 和 (.+)，当前尚未完整配置；请先在设置中填写，再开始翻译。$/u, (match) => `${translateLegacyText(match[1], 'es-ES')} requiere ${translateLegacyText(match[2], 'es-ES')} y ${translateLegacyText(match[3], 'es-ES')}, que no están configurados por completo. Añádelos en los ajustes antes de traducir.`],
    [/^(.+) 需要 (.+)，当前尚未配置；请先在设置中填写，再开始翻译。$/u, (match) => `${translateLegacyText(match[1], 'es-ES')} requiere ${translateLegacyText(match[2], 'es-ES')}, que no está configurado. Añádelo en los ajustes antes de traducir.`],
    [/^输入 (.+)；留空表示尚未配置$/u, (match) => `Introduce ${translateLegacyText(match[1], 'es-ES')}; deja vacío si aún no está configurado`],
...
const jaLegacyPatterns: ReadonlyArray<readonly [RegExp, (match: RegExpExecArray) => string]> = [
    ...
    // 云服务厂商的凭据名称由目录提供，用通用模板兜底；具体模板在前面先匹配。
    [/^(.+) 需要 (.+) 和 (.+)，当前尚未完整配置；请先在设置中填写，再开始翻译。$/u, (match) => `${translateLegacyText(match[1], 'ja-JP')} には ${translateLegacyText(match[2], 'ja-JP')} と ${translateLegacyText(match[3], 'ja-JP')} が必要ですが、まだ完全には設定されていません。翻訳前に設定してください。`],
    [/^(.+) 需要 (.+)，当前尚未配置；请先在设置中填写，再开始翻译。$/u, (match) => `${translateLegacyText(match[1], 'ja-JP')} には ${translateLegacyText(match[2], 'ja-JP')} が必要ですが、まだ設定されていません。翻訳前に設定してください。`],
    [/^输入 (.+)；留空表示尚未配置$/u, (match) => `${translateLegacyText(match[1], 'ja-JP')} を入力してください。未設定の場合は空のままにします`],
...
const koLegacyPatterns: ReadonlyArray<readonly [RegExp, (match: RegExpExecArray) => string]> = [
    ...
    // 云服务厂商的凭据名称由目录提供，用通用模板兜底；具体模板在前面先匹配。
    [/^(.+) 需要 (.+) 和 (.+)，当前尚未完整配置；请先在设置中填写，再开始翻译。$/u, (match) => `${translateLegacyText(match[1], 'ko-KR')}에는 ${translateLegacyText(match[2], 'ko-KR')}과(와) ${translateLegacyText(match[3], 'ko-KR')}이(가) 필요하지만 아직 모두 설정되지 않았습니다. 번역 전에 설정에서 입력하세요.`],
    [/^(.+) 需要 (.+)，当前尚未配置；请先在设置中填写，再开始翻译。$/u, (match) => `${translateLegacyText(match[1], 'ko-KR')}에는 ${translateLegacyText(match[2], 'ko-KR')}이(가) 필요하지만 아직 설정되지 않았습니다. 번역 전에 설정에서 입력하세요.`],
    [/^输入 (.+)；留空表示尚未配置$/u, (match) => `${translateLegacyText(match[1], 'ko-KR')} 입력; 아직 설정하지 않았다면 비워 두세요`],
...
const frLegacyPatterns: ReadonlyArray<readonly [RegExp, (match: RegExpExecArray) => string]> = [
    ...
    // 云服务厂商的凭据名称由目录提供，用通用模板兜底；具体模板在前面先匹配。
    [/^(.+) 需要 (.+) 和 (.+)，当前尚未完整配置；请先在设置中填写，再开始翻译。$/u, (match) => `${translateLegacyText(match[1], 'fr-FR')} nécessite ${translateLegacyText(match[2], 'fr-FR')} et ${translateLegacyText(match[3], 'fr-FR')}, qui ne sont pas entièrement configurés. Ajoutez-les dans les réglages avant de traduire.`],
    [/^(.+) 需要 (.+)，当前尚未配置；请先在设置中填写，再开始翻译。$/u, (match) => `${translateLegacyText(match[1], 'fr-FR')} nécessite ${translateLegacyText(match[2], 'fr-FR')}, qui n’est pas configuré. Ajoutez-le dans les réglages avant de traduire.`],
    [/^输入 (.+)；留空表示尚未配置$/u, (match) => `Saisissez ${translateLegacyText(match[1], 'fr-FR')} ; laissez vide si ce n’est pas encore configuré`],
...
const ruLegacyPatterns: ReadonlyArray<readonly [RegExp, (match: RegExpExecArray) => string]> = [
    ...
    // 云服务厂商的凭据名称由目录提供，用通用模板兜底；具体模板在前面先匹配。
    [/^(.+) 需要 (.+) 和 (.+)，当前尚未完整配置；请先在设置中填写，再开始翻译。$/u, (match) => `Для ${translateLegacyText(match[1], 'ru-RU')} требуются ${translateLegacyText(match[2], 'ru-RU')} и ${translateLegacyText(match[3], 'ru-RU')}, но они настроены не полностью. Укажите их в настройках перед переводом.`],
    [/^(.+) 需要 (.+)，当前尚未配置；请先在设置中填写，再开始翻译。$/u, (match) => `Для ${translateLegacyText(match[1], 'ru-RU')} требуется ${translateLegacyText(match[2], 'ru-RU')}, но это не настроено. Укажите значение в настройках перед переводом.`],
    [/^输入 (.+)；留空表示尚未配置$/u, (match) => `Введите ${translateLegacyText(match[1], 'ru-RU')}; оставьте пустым, если ещё не настроено`],
