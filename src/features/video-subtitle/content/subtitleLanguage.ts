 ...
 * 主要内容：中文目标把明确的简体与繁体中文都视为目标语言；其他目标沿用全局保守预检，只在书写体系或统计结果明确时跳过。
 * 模块边界：纯文本判定，不读取配置、不发起翻译，也不处理排除语言；短文本或不确定结果一律继续翻译。
 */
...
export function isVideoSubtitleInTargetLanguage(text: string, targetLanguage: string): boolean {
    if (getChineseScript(targetLanguage)) {
        return isClearlyTargetLanguage(text, 'zh-Hans') || isClearlyTargetLanguage(text, 'zh-Hant');
    }
