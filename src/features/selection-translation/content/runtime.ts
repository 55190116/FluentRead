 ...
 * 主要内容：缓存 ContentScriptContext，维护 mountRequestId、实例与 Shadow UI 句柄，通过 modal host 控制器承接组件选区，并在功能关闭或请求过期时恢复 DOM 所有权并移除。
...
export function mountSelectionTranslator(ctx?: ContentScriptContext) {
  ...
  mountingPromise = createVueShadowUi(contentScriptContext, {
    ...
    mode: 'closed',
    props: {
      onSelectionRangeChange: (range: Range | null) => modalDialogHost?.placeForRange(range),
    },
  }).then((ui) => {
