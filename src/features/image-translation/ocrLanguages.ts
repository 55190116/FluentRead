 ...
 * 主要内容：包含 eng、chi_sim、chi_tra、jpn 类型与展示元数据、推荐简繁中英集合、getRequiredImageOcrLanguages 选择规则和 normalizeImageOcrLanguageCodes 白名单去重。
...
export type ImageOcrLanguageCode = 'eng' | 'chi_sim' | 'chi_tra' | 'jpn';
...
export function getRequiredImageOcrLanguages(sourceLanguage: string): ImageOcrLanguageCode[] {
    ...
    // 自动源语言同时覆盖简繁与英文，避免默认配置把繁体识别成简体后丢失脚本信息。
