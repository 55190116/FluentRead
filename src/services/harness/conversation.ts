...
export function createHarnessConversationRuntime(deps: ConversationDependencies) {
    ...
    return {
        async run(request: ReadingRequest, signal: AbortSignal, onProgress?: (progress: ReadingProgress) => void, privateContext = false): Promise<ReadingResponse> {
            ...
            await save();
            notify({kind: 'session', sessionId: persistent ? session.id : undefined, persistent, warning});
            if (signal.aborted) {
                turn.status = 'stopped';
                await save();
                return CANCELLED;
            ...
            try { response = await deps.runtime.run(restoredRequest, signal, publish); }
            ...
            return response.success ? {
                ...response, ...(persistent ? {sessionId: session.id} : {}), ...(warning ? {persistenceWarning: warning} : {}),
            } : response;
        },
    };
}
